...
Event::Event(std::string name, std::string place, Date start_date, Date end_date, Date reminder, std::string username, bool done) {
	...
	this->id = calculate_id();
	this->rem_id = calculate_rem_id() + std::to_string(counter++);
...
std::string Event::calculate_id() {
	std::string new_month = "0";

	if (start_date.getMonth() <= 9)
		new_month = "0" + std::to_string(start_date.getMonth());
	else
		new_month = std::to_string(start_date.getMonth());

	std::string new_day = "0";

	if (start_date.getDay() <= 9)
		new_day = "0" + std::to_string(start_date.getDay());
	else
		new_day = std::to_string(start_date.getDay());

	std::string new_hour = "0";

	if (start_date.getHour() <= 9)
		new_hour = "0" + std::to_string(start_date.getHour());
	else
		new_hour = std::to_string(start_date.getHour());

	std::string new_minute = "0";

	if (start_date.getMinute() <= 9)
		new_minute = "0" + std::to_string(start_date.getMinute());
	else
		new_minute = std::to_string(start_date.getMinute());

	//this->id = std::to_string(start_date.year) + std::to_string(start_date.month)+ std::to_string(start_date.day)+ std::to_string(start_date.hour)+ std::to_string(start_date.minute);
	std::string  new_year = std::to_string(start_date.getYear());
	return new_year + new_month + new_day + new_hour + new_minute;
}

std::string Event::calculate_rem_id() {
	std::string year_rem = std::to_string(this->reminder.getYear());

	std::string month_rem = "0";
	if (this->reminder.getMonth() <= 9)
		month_rem = "0" + std::to_string(this->reminder.getMonth());
	else
		month_rem = std::to_string(this->reminder.getMonth());

	std::string day_rem = "0";
	if (this->reminder.getDay() <= 9)
		day_rem = "0" + std::to_string(this->reminder.getDay());
	else
		day_rem = std::to_string(this->reminder.getDay());
	return year_rem + month_rem + day_rem;
}
